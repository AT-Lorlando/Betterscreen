...
func TestContextLauncher(t *testing.T) {
	...
	if c.InSession {
		t.Error("STY vide doit donner InSession=false")
	}
	if c.Window != -1 {
		t.Errorf("Window = %d, want -1 quand hors session", c.Window)
	}
}
...
func TestContextInSession(t *testing.T) {
	...
	if !c.InSession {
		t.Error("STY défini doit donner InSession=true")
	}
...
func TestContextInSessionNoWindow(t *testing.T) {
	...
	if c.Window != -1 {
		t.Errorf("Window = %d, want -1 quand WINDOW absent/invalide", c.Window)
	}
}

func TestContextInSessionInvalidWindow(t *testing.T) {
	env := map[string]string{"STY": "9.work", "WINDOW": "abc"} // non numérique
	c := contextFrom(func(k string) string { return env[k] })
	if c.Window != -1 {
		t.Errorf("Window = %d, want -1 quand WINDOW est invalide", c.Window)
	}
}
